...
We added several new columns after the version years columns. One of these is the `Swadesh` columns (Boolean `true`/`false`), indicating whether the entries are part of the Swadesh items (`true`) or not (`false`)[^1] [see @holleli1980, 141-143]. From this table, it is then possible to easily filter out the Swadesh items, something that is not possible in the PDF version, since the NBL table does not directly include a column marking which items are from the Swadesh list; we then hand-coded this Swadesh column.
